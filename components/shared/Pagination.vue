<template>
  <div
    class="flex justify-center items-center gap-2 border-x px-2 border-base-300"
  >
    <nav>
      <button
      ...
      <template v-for="pageNumber in displayedPages" :key="pageNumber">
        <button
          class="pagination-btn"
          :class="{ 'active-pagination-btn': pageNumber === currentPage }"
    ...
    <p class="text-xs hidden sm:block">{{ totalItems }} itens</p>
  </div>
</template>
